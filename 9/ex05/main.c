...
void increment_timer_init() {
	//Set timer 1 to CTC mode with OCR1A as top and 1024x prescaler and interrupts
	//This will generate interrupts at intervals of 1s
	SREG |= (1 << SREG_I);
...
ISR(TIMER0_OVF_vect) {
	...
	i2c_write(255);
	i2c_write(0);
	if ((digit != 0) || (digit / 10 != 0) || (digit_position == 0)) {
		wait_i2c_ready();
		i2c_start();
		i2c_write(IO_EXP_ADDR | TW_WRITE);
